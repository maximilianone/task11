...
public class MainController implements Patterns {
    ...
    private void addTask() {
        ...
        try {
            mainView.displayMessage((String) CommandList.ADD_TASK.getCommand().execute(connection, request));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    private void showEmployeeTasks() {
        try {
            Integer employeeNumber = Integer.parseInt(inputValue(MainView.REQUEST_EMPLOYEE_NUMBER,NUMBER_PATTERN));
            ResultSet resultSet = (ResultSet) CommandList.SHOW_TASK.getCommand().execute(connection, employeeNumber);
            mainView.displayResultSet(resultSet);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private void deleteEmployee() {
        try {
            Integer employeeNumber = Integer.parseInt(inputValue(MainView.REQUEST_EMPLOYEE_NUMBER,NUMBER_PATTERN));
            CommandList.DELETE_EMPLOYEE.getCommand().execute(connection, employeeNumber);
